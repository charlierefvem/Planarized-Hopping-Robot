...
function ode_fn!(du,u,p,t)
     τ₁ =  0
     τ₃ =  0
     A = [p.Lᵦ^2*p.m₁ + 2*p.Lᵦ*(p.Lₐ-p.L₂)*p.m₁*cos(u[2]) + (p.L₂-p.Lᵦ)^2*p.m₂ + (p.L₂-p.Lₐ)^2*p.m₁        p.Lᵦ^2*p.m₁ + (p.L₂-p.Lᵦ)^2*p.m₂ + p.Lᵦ*(p.Lₐ-p.L₂)*p.m₁*cos(u[2]);
          p.Lᵦ^2*p.m₁ +   p.Lᵦ*(p.Lₐ-p.L₂)*p.m₁*cos(u[2]) + (p.L₂-p.Lᵦ)^2*p.m₂                             p.Lᵦ^2*p.m₁ + (p.L₂-p.Lᵦ)^2*p.m₂];
    
     B = [p.Lᵦ*p.m₁*u[6]*(p.Lₐ-p.L₂)*(2*u[5] + u[6])*sin(u[2]) + p.Lᵦ*p.g*(p.m₁+p.m₂)*sin(u[1] + u[2]) - p.L₂*p.g*p.m₂*sin(u[1] + u[2]) + (p.Lₐ-p.L₂)*p.g*p.m₁*sin(u[1]) - p.c₁*u[5] + τ₁;
         -p.Lᵦ*p.m₁*u[5]^2*(p.Lₐ-p.L₂)*sin(u[2])               + p.Lᵦ*p.g*(p.m₁+p.m₂)*sin(u[1] + u[2]) - p.L₂*p.g*p.m₂*sin(u[1] + u[2])                                  - p.c₂*u[6] + τ₃]
...
@gif for i ∈ 1:n
     ...
     plot(x[:,i],y[:,i],ylim = [-10,10],xlim=[-15,15],xlabel="X Position", ylabel="Y Position", 
        markercolors = [:black, :red, :black, :red, :blue], shape = [:none,:circle, :cross, :circle, :circle],
        markersize = [3,8,6,8,16], linewidth = 4, title = round(hi*100)/100, aspect_ratio=:equal)
end every 1
