...
function ode_fn!(du,u,p,t)
     ...
     M = [p.m₁*p.L₁^2 + p.m₂*(p.Lₐ^2 + p.L₂^2 + 2*p.Lₐ*p.L₂)      p.m₂*(p.L₂^2 + p.Lₐ*p.L₂)      (p.m₁*p.L₁+p.m₂*p.Lₐ) + p.m₂*p.L₂    (p.m₁*p.L₁+p.m₂*p.Lₐ)*u[1] + p.m₂*p.L₂*(u[1]+u[2]);
         p.m₂*(p.L₂^2 + p.Lₐ*p.L₂)                               p.m₂*p.L₂^2                               p.m₂*p.L₂                                    p.m₂*p.L₂*(u[1]+u[2]);
         (p.m₁*p.L₁+p.m₂*p.Lₐ) + p.m₂*p.L₂          p.m₂*p.L₂                  p.m₁ + p.m₂                                                0;
         (p.m₁*p.L₁+p.m₂*p.Lₐ)*u[1] + p.m₂*p.L₂*(u[1]+u[2])          p.m₂*p.L₂*(u[1]+u[2])                   0                                                          p.m₁ + p.m₂];
     V = [p.c₁*u[5] + p.g*p.m₂*p.L₂*(u[1]+u[2]) + p.g*(p.m₁*p.L₁ + p.m₂*p.Lₐ)*sin(u[1]);
          p.c₂*u[6] + p.g*p.m₂*p.L₂*(u[1]+u[2]);
          0;
          p.g*(p.m₁ + p.m₂)];
...
### Initial Conditions ###
x₀ = [-.9, .9, 0, 0, -.3, .8, -.5, 7];
#x₀ = [0, 0.1, 1, -10, 0, 0, 0, 0];
#x₀ = [-0.04661698521390433 0.06345661598167297 1.3412203060434433 -31.35986906309882 -0.9597418087104923 1.116835444394521 3.211953613334984 -21.123039299828]
...
@gif for i ∈ 1:n
     plot(x[:,i],y[:,i],ylim = [-10,20],xlim=[-15,15],xlabel="X Position", ylabel="Y Position", 
        ...
        markersize = [2,2,1,2,4], linewidth = 1, title = "Linear Single Leg In Flight",aspect_ratio=:equal)
end every 1
